document.addEventListener("DOMContentLoaded", () => {
    ...
    async function pollSimulationStatus(simId) {
        ...
        try {
            const response = await fetch(`/api/simulate/status/${simId}`);
            ...
            if (status.gaFinished && status.acoFinished) {
                stopPolling(`Simulation finished with overall status: ${status.overallStatus}`, status.overallStatus.toLowerCase());

                // Always try to update results from backend if available, even if overall status is ERROR
                if (status.result) {
                ...
                // If not yet finished, show the running message
                showResultsMessage("Simulation is running...", "info");
            }
